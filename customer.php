...
<h3 class="w3-center">CUSTOMERS</h3>
    <!-- Start Shop Page  --><br>
      <div id="div" style="text-align: center;" >
              <table class='data-table' style='border: solid 1px black;' align="center">
              <?php
              echo "<tr><th>Customer ID</th><th>Firstname</th><th>Lastname</th><th>Email address</th><th>PhoneNo</th><th>Membership</th></tr>";
              $sql = "SELECT * FROM Customer;";
              $result = mysqli_query($connect, $sql);

              while($row = mysqli_fetch_assoc($result))
              {
                  echo "<tr><td>{$row["cust_id"]}</td><td>{$row["fName"]}</td><td>{$row["lName"]}</td>
                  <td>{$row["email"]}</td><td>{$row["phone_no"]}</td><td>{$row["membership"]}</td></tr>";
              }
              ...
              <br>
              <button class="button"><a class="nav-link" href="cust-search.php">Search 3 First Customer</a></button>
              <button class="button"><a class="nav-link" href="cust-insert.php">Insert New Customer</a></button>
              <button class="button"><a class="nav-link" href="cust-update.php">Update Customer Info</a></button>
              <button class="button"><a class="nav-link" href="cust-3D.php">3D Glasses Elligiblity</a></button>
              <br><br>
      </div>
    <!-- End Shop Page -->
