...
<h3 class="w3-center">BOOKINGS</h3>
    <!-- Start Shop Page  --><br>
      <div id="div" style="text-align: center;" >
              <table class='data-table' style='border: solid 1px black;' align="center">
              <?php
              echo "<tr><th>Booking ID</th><th>Customer ID</th><th>Movie ID</th><th>Movie Name</th><th>Movie Date</th><th>Movie Time</th><th>Seat No</th><th>Hall</th></tr>";
              $sql = "SELECT * FROM Booking;";
              $result = mysqli_query($connect, $sql);

              while($row = mysqli_fetch_assoc($result))
              {
                  echo "<tr><td>{$row["booking_ID"]}</td><td>{$row["cust_ID"]}</td><td>{$row["movie_id"]}</td>
                  <td>{$row["movie_name"]}</td><td>{$row["movie_date"]}</td><td>{$row["movie_time"]}</td><td>{$row["seat_no"]}</td><td>{$row["hall"]}</td></tr>";
              }

              ?>
              </table>
              <br>
              <button class="button"><a class="nav-link" href="booking-search.php">Search Booking</a></button>
              <button class="button"><a class="nav-link" href="booking-insert.php">Insert New Booking</a></button>
              <button class="button"><a class="nav-link" href="booking-update.php">Update Booking Info</a></button>
              <br><br>
      </div>
    <!-- End Shop Page -->
